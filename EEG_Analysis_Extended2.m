...
% Camera Position for 3D cluster plots
CAMERAPOSITION=[-5.8955  -74.7660    2.4542];
...
%% Change Electrical and Acceleration selection to disclude >max-100 values
suggestion=true(Electrical.n,1);
kernel=true(round(10*Electrical.fs),1); % Kernel of 10 s
for ii=1:Settings.nChannel
    suggestion=suggestion&~conv(abs(Electrical.CH1234(:,ii))>max(Electrical.CH1234(:,ii))-100,kernel,'same');
end
Electrical.points=logical2points(points2logical(Electrical.points,Electrical.n)&suggestion);
Electrical.points=round(Electrical.points/5)*5;
Electrical.points(:,1)=Electrical.points(:,1)+1;
Electrical.points=Electrical.points(Electrical.points(:,2)-Electrical.points(:,1)+1>=Electrical.nEpochLength,:);
Acceleration.points=round([Electrical.points(:,1)+EleAccMultiple,Electrical.points(:,2)]/5);
...
Epoch_Dataset=[Epoch.BandPowers{:},...
               Epoch.RMS{:},...
               Epoch.DELTA{:},Epoch.THETA{:},Epoch.DELTA2{:},Epoch.THETA2{:},...
               Epoch.CC{triu(true(Settings.nChannel),1)},Epoch.lag{triu(true(Settings.nChannel),1)},...
               Epoch.DynAcc];    
...
xlabel('Length Acceleration (g)','Interpreter','latex'),ylabel('Epoch probability','Interpreter','latex')
...
xlabel('NREM ratio ($\Delta=\frac{\delta*\alpha}{\beta*\gamma}$)','Interpreter','latex'),ylabel('Epoch probability','Interpreter','latex')
...
Epoch.REMQA{1}=Epoch.REMQA{2}; %use probabilities of right-M for right-S
Epoch.REMQA{4}=Epoch.REMQA{3}; %use probabilities of left-M for left-S
...
xlabel('REM ratio ($\Theta=\frac{\theta^2}{\delta*\alpha}$)','Interpreter','latex'),ylabel('Epoch probability','Interpreter','latex')
...
Plot_points=[Electrical.points(1,1),Electrical.points(1,2)-10000];
...
SaveFolder='C:\Users\enzo\Downloads\Study\Current Courses\BEP\Data\EEG\';
save([SaveFolder,Name{1},'_',Genotype{1},'_',Day{1},'_Analyzed'],'DataTable','Acceleration','Electrical','Acceleration_P','Electrical_P','Decision_Summary','Epoch','PSD','Settings','StateSummary','FFT','CC','Mouse','Epoch_Dataset','check_D3')
         
DataTable_cell{E_index}=DataTable;

if E_index<nE
    clearvars -except E_Name nE Settings E_index DataTable_cell FFT CC Plot Mouse check_D3
end
...
save([SaveFolder,'DataTable'],'DataTable')
save([SaveFolder,'Settings'],'Settings')